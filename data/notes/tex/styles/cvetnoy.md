...
**Подпись сначала двигают, а не закрывают ею чертёж.** Место ищется так, чтобы
буква не легла на линию: наружу дуги, в сторону от хорды, к свободному концу прямой.
Белый кружок под подписью — запасной ход для тесных мест, где двигать некуда,
а не способ по умолчанию:

```latex
pl/.style={circle,fill=white,fill opacity=0.85,text opacity=1,inner sep=0.6pt},
```

Ставится в списке опций узла **последним**: цвет, указанный после него, задаёт
и заливку тоже, и подпись превращается в сплошной цветной кружок с текстом того же
цвета. По той же причине подложку нельзя встроить в стили `lb` и `lbs` — они
раскрываются раньше цвета.
